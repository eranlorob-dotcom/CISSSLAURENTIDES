...
def build_digest_with_claude(client, sci_items, media_items, meta):
    contexte_chercheur = meta.get(
        "contexte_chercheur",
        "un chercheur en santé publique et aménagement du territoire",
    )
    sci_block = "\n\n".join(
    ...
    prompt = f"""Tu prépares un digest de veille hebdomadaire pour {contexte_chercheur}.
...
def send_email(html_body, subject, gmail_user, gmail_app_password, recipient, titre_digest):
    ...
    <body style="font-family: 'Segoe UI', Arial, sans-serif; color: #1a1a1a; max-width: 700px; margin: auto;">
      <div style="background: #140C74; color: white; padding: 16px 24px; border-radius: 6px 6px 0 0;">
        <h1 style="margin:0; font-size: 20px;">{titre_digest}</h1>
...
def main():
    ...
    meta = keywords_cfg.get("meta", {})
    titre_digest = meta.get("titre_digest", "Veille scientifique")

    print("Filtrage et résumé via Claude...")
    client = anthropic.Anthropic(api_key=anthropic_key)
    html_digest = build_digest_with_claude(client, sci_items, media_items, meta)
